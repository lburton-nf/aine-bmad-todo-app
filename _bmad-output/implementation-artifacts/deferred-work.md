...
- **DB_PATH default is container-only.** `/data/todos.db` is a Linux/Docker absolute path. Local macOS/dev runs need `DB_PATH` explicitly set (e.g., `DB_PATH=./data/todos.db`). Story 2.1 (db.ts) is the natural place to surface this — at minimum, an error message that mentions the env var if the path is unwritable.
